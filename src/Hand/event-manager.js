...
AFRAME.registerComponent('event-manager', {
  ...
  init: function () {
    ...
    // Añadir la detección de gesto de pulgar arriba
    this.thumbUpGestureDetected = false;
    this.el.sceneEl.addEventListener('thumbupdetected', this.onThumbUp.bind(this));
  },
  ...
  onThumbUp: function () {
    // Cambiar al modo oscuro cuando se detecta el pulgar arriba
    var darkModeButtonEl = this.darkModeButtonEl;
    if (this.el.sceneEl.is('starry')) {
      darkModeButtonEl.setAttribute('button', 'label', 'Dark Mode');
      this.el.sceneEl.setAttribute('environment', {preset: 'default'});
      this.el.sceneEl.removeState('starry');
    } else {
      darkModeButtonEl.setAttribute('button', 'label', 'Light Mode');
